...
partial model PartialConductionElement "Element with quasi-stationary mass and heatport and undetermined heat transfer coefficient"
  ...
  parameter Boolean useAverageHeatflow = false "Apply 2nd order approach for faster convergence?";
...
equation
  ...
  if useAverageHeatflow then
    ...
    assert(noEvent(DT1*DT2 > 0), "Temperature Crossing! Temperature differences have different sign", AssertionLevel.warning);
