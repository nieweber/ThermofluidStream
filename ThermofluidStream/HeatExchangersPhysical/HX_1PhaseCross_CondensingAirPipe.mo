...
model HX_1PhaseCross_CondensingAirPipe "Heat Exchanger: Single Phase Fluid in Cross Flow and Condensing Air in Pipe Flow"
  extends HXcrossIcon;
  ...
  replaceable package Medium_PipeChannel = ThermofluidStream.Media.myMedia.Air.MoistAir constrainedby ThermofluidStream.Media.myMedia.Interfaces.PartialCondensingGases
                                                                        "Medium model in pipe channel (B)"
    annotation (choicesAllMatching=true, Dialog(group = "Medium definitions"));
...
//  parameter SI.Temperature T_cross_initial = 300   "Initial Cross Channel Temperature";
  ...
  parameter SI.Temperature T_0_cross=293.15      "Initial Temperature"
...
/*  ThermofluidStream.Processes.FlowResistance flowResistanceA[nCells](
  ...
  ThermofluidStream.Interfaces.Outlet outlet_pipe_water(redeclare package Medium = Medium_Water) annotation (Placement(transformation(
        extent={{-10,-10},{10,10}},
        rotation=90,
        origin={32,100}), iconTransformation(extent={{-10,-10},{10,10}}, origin={40,98},
        rotation=90)));
  ThermofluidStream.Boundaries.TerminalSource terminalSource(redeclare package Medium = ThermofluidStream.Media.myMedia.Water.StandardWater, each p_0=100100) annotation (Placement(transformation(
        extent={{-10,-10},{10,10}},
        rotation=90,
        origin={20,-46})));
protected
...
//  end for;
  ...
  for i in 1:nCells-1 loop
    connect(hxCellPair[i].outlet_pipe_water, hxCellPair[i + 1].inlet_pipe_water);
  end for;
  ...
  connect(splitterN.outlets, hxCellPair.inlet_Cross)
    annotation (Line(
      ...
      color={28,108,200},
      thickness=0.5));
  connect(hxCellPair.outlet_Cross, junctionN.inlets)
    annotation (Line(
      points={{20,0},{60,0}},
      color={28,108,200},
      thickness=0.5));
...
/*connect(flowResistanceA.outlet, junctionN.inlets) annotation (Line(
  ...
  connect(hxCellPair[nCells].outlet_Pipe, outlet_Pipe)
    annotation (Line(
      ...
      color={28,108,200},
      thickness=0.5));
  connect(terminalSource.outlet, hxCellPair[1].inlet_pipe_water) annotation (Line(
      points={{20,-36},{20,-28},{8,-28},{8,-20}},
      color={28,108,200},
      thickness=0.5));
  connect(hxCellPair[nCells].outlet_pipe_water, outlet_pipe_water) annotation (Line(
      points={{8,19.6},{8,70},{32,70},{32,100}},
      color={28,108,200},
      thickness=0.5));
  annotation (
    Icon(
...
end HX_1PhaseCross_CondensingAirPipe;
