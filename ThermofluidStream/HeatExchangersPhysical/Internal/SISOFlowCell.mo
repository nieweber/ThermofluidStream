...
partial model SISOFlowCell "Base Model with basic flow eqautions for SISO"
  ...
  replaceable package Medium = ThermofluidStream.Media.myMedia.Interfaces.PartialMedium
    "Medium model"
    annotation (choicesAllMatching=true, Dialog(group = "Medium definitions"), Documentation(info="<html>
    <p>Medium package used in the Component. Make sure it is the same as the components connected to both ports are using.</p>
      </html>"));

  parameter ThermofluidStream.Utilities.Units.Inertance L=dropOfCommons.L "Inertance of the flow" annotation (Dialog(tab="Advanced"));
  ...
  parameter Modelica.Units.SI.MassFlowRate m_flow_0=0 "Initial value for m_flow" annotation (Dialog(
      tab="Initialization",
      group="Mass flow",
      enable=(initM_flow == InitializationMethods.state)));
  parameter ThermofluidStream.Utilities.Units.MassFlowAcceleration m_acceleraton_0=0 "Initial value for der(m_flow)" annotation (Dialog(
      tab="Initialization",
      group="Mass flow",
  ...
  parameter Boolean clip_p_out "If false, set dr_corr to zero"
    annotation(Dialog(tab="Advanced"));
  parameter Modelica.Units.SI.Pressure p_min=dropOfCommons.p_min "Minimal steady-state output pressure" annotation (Dialog(tab="Advanced", enable=clip_p_out));

  ThermofluidStream.Interfaces.Inlet inlet(redeclare package Medium = Medium) annotation (Placement(transformation(extent={{-120,-20},{-80,20}}), iconTransformation(extent={{-120,-20},{-80,20}})));
  ThermofluidStream.Interfaces.Outlet outlet(redeclare package Medium = Medium) annotation (Placement(transformation(extent={{80,-20},{120,20}}), iconTransformation(extent={{80,-20},{120,20}})));

  Modelica.Units.SI.MassFlowRate m_flow(stateSelect=m_flowStateSelect)=inlet.m_flow "mass flow through component";

// changing pressure calculation
  Modelica.Units.SI.Pressure dr_corr;
                     // delta = out - in
  Modelica.Units.SI.Pressure dp;
                // delta = out - in

  Modelica.Units.SI.MassFlowRate dm_flow;

protected
  outer ThermofluidStream.DropOfCommons dropOfCommons;

  // inlet state quantities
  Modelica.Units.SI.Pressure p_in=Medium.pressure(inlet.state) "pressure of medium entering";
  Modelica.Units.SI.SpecificEnthalpy h_in=Medium.specificEnthalpy(inlet.state) "enthaply of medium enetering";
  ...
  Modelica.Units.SI.Pressure p_out "pressure of medium exiting";
  Modelica.Units.SI.SpecificEnthalpy h_out "enthaply of medium exiting";
...
equation
  ...
  //outlet.m_flow = -max(inlet.m_flow - dm_flow, 0.0);
  //dm_flow = 0;

  inlet.m_flow + dm_flow + outlet.m_flow = 0;
