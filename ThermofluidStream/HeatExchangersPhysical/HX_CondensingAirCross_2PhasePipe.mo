...
model HX_CondensingAirCross_2PhasePipe "Heat Exchanger: Air with condensing Water Vapor in Cross Flow and 2 Phase Fluid in Pipe Flow"
  extends HXcrossIcon;
...
public
  ...
  ThermofluidStream.Interfaces.Outlet outlet_cross_water(redeclare package Medium = Medium_Water) annotation (Placement(transformation(
        extent={{-10,-10},{10,10}},
        rotation=0,
        origin={102,-30}), iconTransformation(extent={{-10,-10},{10,10}}, origin={100,-40})));
  ThermofluidStream.Topology.JunctionN junction_water(redeclare package Medium = ThermofluidStream.Media.myMedia.Water.StandardWater, N=nCells) annotation (Placement(transformation(extent={{60,-40},{80,-20}})));
  ThermofluidStream.Boundaries.TerminalSource terminalSource[nCells](redeclare package Medium = ThermofluidStream.Media.myMedia.Water.StandardWater, each p_0=100100) annotation (Placement(transformation(extent={{-80,-30},{-60,-10}})));
protected
...
//  end for;
  ...
  connect(hxCellPair[nCells].outlet_Pipe, outlet_Pipe)
    annotation (Line(
      ...
      color={28,108,200},
      thickness=0.5));
  connect(junction_water.outlet, outlet_cross_water) annotation (Line(
      points={{80,-30},{102,-30}},
      color={28,108,200},
      thickness=0.5));
  connect(hxCellPair.outlet_cross_water, junction_water.inlets) annotation (Line(
      points={{20,-8},{40,-8},{40,-30},{60,-30}},
      color={28,108,200},
      thickness=0.5));
  connect(hxCellPair.outlet_Cross, junctionN.inlets) annotation (Line(
      points={{20,0},{60,0}},
      color={28,108,200},
      thickness=0.5));
  connect(terminalSource.outlet, hxCellPair.inlet_cross_water) annotation (Line(
      points={{-60,-20},{-40,-20},{-40,-8},{-20,-8}},
      color={28,108,200},
      thickness=0.5));
  annotation (
    Icon(
...
end HX_CondensingAirCross_2PhasePipe;
